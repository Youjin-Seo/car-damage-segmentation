    ...
    if arg.train:
        ...
        if (arg.label == "all") & (arg.task == "damage"):
            ...
            epochs = [1,8,5,9]  ## 각 label에 대한 epoch. label마다 epoch 횟수를 달리 하고 있다. 현재는 사용하지 않음
                                ## 10/29) 모델 돌려본 결과 Scratched가 그나마 학습이 잘 되기 때문에 epoch 수를 달리한 듯 
    ...
    if arg.eval:
        ...
        if arg.task == 'damage':
            evaluation = Evaluation(
                        ...
                        size = 256, 
                        model = model, 
                        # model은 Unet(encoder="resnet34",pre_weight='imagenet',num_classes=n_cls)를 의미
                        ...
                        weight_paths = ['../data/weight/resnet34-333f7ec4.pth','../data/weight/resnet34-333f7ec4.pth','../data/weight/resnet34-333f7ec4.pth','../data/weight/resnet34-333f7ec4.pth'],
                        device = 'cuda',
                        batch_size = 64, 
