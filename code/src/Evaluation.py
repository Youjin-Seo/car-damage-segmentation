...
class Evaluation():
    def __init__(self, 
        ...
        if len(self.weight_paths) > 1: ## damage
    ...
    def load_model(self, model, weight_path):
        ...
        #   checkpoint[key.replace(key, ST+key)] = checkpoint[key]
        #   del checkpoint[key]
          
        #   # if 'model.encoder.' in key:
        #   #   checkpoint[key.replace('model.encoder.','')] = checkpoint[key]
        #   #   del checkpoint[key]
    ...
    def validation(self, model, epoch, step, data_loader):
        ...
        with torch.no_grad():
            ...
            for step, (images, masks, img_ids) in tqdm.tqdm(enumerate(data_loader)):
                ...
                outputs = torch.argmax(outputs, dim=1).detach().cpu().numpy() # 예측값
                # QQ 하나의 row에서 가장 큰 값만 저장 

                masks = masks.detach().cpu().numpy() # 실제값
                

                # 각 이미지 별 실측 vs 예측의 IOU
                for i, img_id in enumerate(img_ids):
                    h = np.zeros((n_class, n_class))        
                    h += add_hist(h, masks[i], outputs[i], n_class=n_class)
                    # h : 여기의 for문을 통해서 이미지 하나 + 예측 하나의 confusion matrix 제작
                    # 이거를 누적합 --> for문이 다 돌아가면 h는 최종 하나의 label에 속하는 데이터들의 confusion matrix
                ...
                # 위의 h : 하나의 class에 속한 이미지 사이의
                # 여기의 hist : 4개의 레이블 전체 합
